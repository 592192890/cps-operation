...
function AdActivitiesAuditContext($state,$stateParams,$cookieStore,$timeout,apiService,webStorageCache,$modal) {
	...
	this.approvePass = function(approveType) {
		...
		this.queryModel.entityDTO.approveResult = approveType;
		
		this.queryModel.entityDTO.activityId = webStorageCache.get('activityId');
		
		
		var promise = apiService.post(ApiDefines.APPROVE_ACTIVITY_INFO, angular
				.extend(this.queryModel));
		promise.then(function(result) {
			if ('1' == result.code) {
...
AdActivitiesAuditContext.extend(ContextSupport);
